...
local function redir_shell_command(cmd, lines, vertical, stderr_p, stream)
  local shell_cmd = {
    "sh",
    "-c",
    cmd,
    ...
    stdout = function(_err, stdout)
      vim.schedule_wrap(function()
        if stdout ~= nil then
          ...
          if stream == true then
            vim.api.nvim_put({ stdout }, "l", false, true)
          else
...
local function redir(args, stream)
  local cmd = args.args
  ...
  if cmd:sub(1, 1) == "!" then
    ...
    cmd = cmd:sub(2)
    redir_shell_command(cmd, lines, vertical, stderr_p, stream)
  else
...
--- Setup function for enabling redir commands.
function plugin.setup()
  vim.api.nvim_create_user_command("Sredir", function(args)
    log.info("Sredir")
    redir(args, true)
  end, {
    nargs = "+",
  ...
  vim.print("something els neinsde")
end
