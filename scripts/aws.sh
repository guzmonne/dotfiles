...
usage() {
  printf "\n\033[4m%s\033[0m\n" "Usage:"
  printf "  aws [OPTIONS] [COMMAND] [COMMAND_OPTIONS]\n"
  printf "  aws -h|--help\n"
  printf "  aws -v|--version\n"
  printf "\n\033[4m%s\033[0m\n" "Commands:"
  cat <<EOF
  auth .... Sets a new AWS Profile
EOF
...
auth_usage() {
  ...
  printf "    Profile to set\n"
  printf "  -h --help\n"
  printf "    Print help\n"
}
parse_auth_arguments() {
  while [[ $# -gt 0 ]]; do
    case "${1:-}" in
      -h|--help)
        auth_usage
...
run() {
  ...
  case "$action" in
    "auth")
      ...
      ;;
    "")
      printf "\e[31m%s\e[33m%s\e[31m\e[0m\n\n" "Missing command. Select one of " "auth" >&2
      usage >&2
      exit 1
      ;;
